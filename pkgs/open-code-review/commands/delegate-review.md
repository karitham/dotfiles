...
- If `ocr` is not found, it is already on PATH via nix (`open-code-review` package).
...
Automatically fix High and Medium issues that are safe and well-defined when the user asks for fixes; otherwise report them.
