...
Your goal is to take a stack trace (from text, a file, a URL, or an issue tracker) and provide a deterministic explanation of _why_ the code failed, along with the specific inputs or state required to reproduce it.
...
- **Crucial:** Use LSP tools to inspect the definitions of variables involved at the crash site.
  - _Example:_ If `user.Process()` panicked, check the definition of `user`. Is it a pointer? interface? nullable?
