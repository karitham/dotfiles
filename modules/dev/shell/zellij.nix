...
lib.mkIf osConfig.dev.shell.enable {
  ...
  # jj-review: layout for the jj-review.nu script. Mirrors default.kdl's
  # zjstatus bar and adds an hx/prr split. The prr ref (full GitHub URI)
  # is read from /tmp/jj-review-ref, which jj-review.nu writes before
  # invoking `zellij --layout jj-review`.
  #
  # NOTE: the zjstatus plugin body below is duplicated from default.kdl
  # (kept in sync by hand). TODO: factor into a shared Nix function.
  xdg.configFile."zellij/layouts/jj-review.kdl".text = ''
    layout {
        default_tab_template {
            pane split_direction="vertical" {
                ...
                pane {
                    command "sh"
                    args "-c" "prr edit $(cat /tmp/jj-review-ref); nu"
                }
            }
            pane size=1 borderless=true {
                plugin location="file://${zjstatus}" {
